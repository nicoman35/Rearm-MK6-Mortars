...
NIC_fn_Rearm_MK6 = {
	...
	if !(isNil{_mortarMK6 getVariable "NIC_rearm_MK6_in_progress"}) exitWith {															// exit with rearming message, if rearming in already in progress
		[_mortarMK6] call NIC_fn_Rearme_MK6_in_progress_message;
	};
	private _weapArray = [];
	{
...
NIC_fn_Rearmed_MK6_message = {																											// rearming mortar message
	params ["_mortarMK6", "_TotalRearmTime"];
	_mortarMK6 setVariable ["NIC_rearm_MK6_in_progress", true, false];																	// mutex for rearming in progress
	...
	_mortarMK6 setVariable ["NIC_rearm_MK6_in_progress", nil, false];																	// delete rearming in progress mutex
};	// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 

NIC_fn_Rearme_MK6_in_progress_message = {																								// rearming already in progress message
	params ["_mortarMK6"];
	if (gunner _mortarMK6 != player) then {
		_mortarMK6 groupChat format[localize "STR_NIC_MK6_REARMING_PROGRESS"];
	};
};	// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
